 ...
 */
(function( jq162, undefined ) {

jq162.widget( "cg.combogrid", {
	options: {
		resetButton: false,
		resetFields: null,
		searchButton: false,
	...
	_create: function() {
		var self = this,
			...
			if (self.options.resetButton){
				...
				jq162('.' +self.element.attr('id') +'.cg-resetButton').bind('click.combogrid', function(){
					self.element.val('');
					self.term=self.element.val();
					if(self.options.okIcon){
						jq162('.' +self.element.attr('id') +'.ok-icon').remove();
						jq162('.' +self.element.attr('id') + '.notok-icon').remove();
						if(self.options.resetButton){
							jq162('.' +self.element.attr('id') +'.cg-resetButton').after('<span class="'+ self.element.attr('id') +' notok-icon"></span>');
						}else if(self.options.searchButton){
							jq162('.' +self.element.attr('id') +'.cg-searchButton').after('<span class="'+ self.element.attr('id') +' notok-icon"></span>');
						}else{
					...
					if(self.options.resetFields!=null){
						jq162.each(self.options.resetFields, function() {
						jq162( ''+this ).val('');
				 		});
					} 
				});
			...
			if (self.options.searchButton){
				...
				jq162('.' +self.element.attr('id') +'.cg-searchButton').bind('click.combogrid', function(){
		...
		this.element
			...
			.bind( "keydown.combogrid", function( event ) {
				...
				var keyCode = jq162.ui.keyCode;
				switch( event.keyCode ) {
				case keyCode.LEFT:
					jq162('.' +self.element.attr('id') +'.cg-keynav-prev').trigger('click.combogrid');
					break;
				case keyCode.PAGE_UP:
					self._move( "previousPage", event );
					break;
				case keyCode.RIGHT:
					jq162('.' +self.element.attr('id') +'.cg-keynav-next').trigger('click.combogrid');
					break;
		...
		//ESCAPE needs this workaround
					...
					if(self.options.okIcon){
						if(self.options.resetButton){
							jq162('.' +self.element.attr('id') + '.cg-resetButton').after('<span class="'+self.element.attr('id') +' ok-icon"></span>');
						}else if(self.options.searchButton){
							jq162('.' +self.element.attr('id') +'.cg-searchButton').after('<span class="'+self.element.attr('id') +' ok-icon"></span>');
						}else{
				...
				jq162.ajax({
					...
					success: function( data ) {
						if(data.records==0) {
							...
							response(data.records, data.total, jq162.map( data.rows, function( item ) {
								return item;
							}));
							self.menucombo.activate(jq162.Event({ type: "mouseenter" }), self.menucombo.element.children(".cg-menu-item:first"));
							if(self.options.autoChoose){
								setTimeout(function() {
								self.menucombo._trigger("selected", jq162.Event({ type: "click" }), { item: self.menucombo.active });
		...
		this.menucombo = jq162( "<div></div>" )
			.addClass( "cg-autocomplete" )
			.appendTo( jq162( this.options.appendTo || "body", doc )[0] )
			...
			.mousedown(function( event ) {
				...
				if ( !jq162( event.target ).closest( ".cg-menu-item" ).length ) {
					setTimeout(function() {
						jq162( document ).one( 'mousedown', function( event ) {
							if ( event.target !== self.element[ 0 ] &&
								event.target !== menuElement &&
								!jq162.ui.contains( menuElement, event.target ) ) {
								self.close();
							}
						});
					}, 1 );
	...
	destroy: function() {
		...
		jq162.Widget.prototype.destroy.call( this );
	},

	_setOption: function( key, value ) {
		jq162.Widget.prototype._setOption.apply( this, arguments );
		if ( key === "source" ) {
			this._initSource();
		}
		if ( key === "appendTo" ) {
			this.menucombo.element.appendTo( jq162( value || "body", this.element[0].ownerDocument )[0] );
		}
	...
	_initSource: function() {
		...
		if ( jq162.isArray(this.options.source) ) {
			array = this.options.source;
			this.source = function( request, response ) {
				response( jq162.cg.combogrid.filter(array, request.term) );
			};
	...
	close: function( event ) {
		...
		if ( this.menucombo.element.is(":visible") ) {
			...
			jq162('.' +self.element.attr('id') +'.cg-keynav-next').unbind('click.combogrid');
			jq162('.' +self.element.attr('id') +'.cg-keynav-prev').unbind('click.combogrid');
			jq162('.' +self.element.attr('id') +'.cg-keynav-last').unbind('click.combogrid');
			jq162('.' +self.element.attr('id') +'.cg-keynav-first').unbind('click.combogrid');
	...
	_normalize: function( items ) {
		...
		return jq162.map( items, function(item) {
...
//		ul2.appendTo(ul);
		...
		this._resizeMenu();
		if(this.pos==null){
			ul.position( jq162.extend({
				of: this.element
			}, this.options.position ));
		}
		if ( this.options.autoFocus ) {
			this.menucombo.next( new jq162.Event("mouseover") );
		}
	},
	...
	_renderHeader: function(ul, colModel ) {
		var self = this;
		div = jq162('<div id="cg-divHeader" class="ui-state-default">');
		jq162.each( colModel, function( index, col ) {
		...
		if(this.options.draggable){
			jq162('#cg-divHeader').css("cursor","move");
		}
		jq162(".cg-colHeader-label").bind('click.combogrid',function(){
			self.options.sord="";
			self.cssCol = "";
			value = jq162(this).attr('id');
			self.cssCol = value;
	...
	_renderMenu: function( ul, items, colModel ) {
		var self = this;
		jq162.each( items, function( index, item ) {
	...
	_renderItem: function( ul, item, colModel) {
		var self = this;
		this.rowNumber++;
		div = jq162("<div class='cg-colItem'>");
		jq162.each( colModel, function( index, col ) {
		if(col.width==undefined){col.width=100/colModel.length;}
		if(col.align==undefined){col.align="center";}
		var hide = "";
    ...
    jq162("<div style='width:"+col.width+self.options.munit+";"+hide+" text-align:"+col.align+"' class='cg-DivItem'>"+ colItem +"</div>").appendTo(div);
		});
		div.append("</div>");
		if(self.options.alternate){
			if(this.rowNumber%2==0){
					return jq162( "<div class='cg-comboItem-even'></div>" ).data( "item.combogrid", item ).append(div).appendTo(ul);
				} else {
					return jq162( "<div class='cg-comboItem-odd'></div>" ).data( "item.combogrid", item ).append(div).appendTo(ul);
				}
		}else{
			return jq162( "<div class='cg-comboItem'></div>" ).data( "item.combogrid", item ).append(div).appendTo(ul);
		}
	},
	_renderPager: function( ul, records, total ) {
		...
		div = jq162("<div class='cg-comboButton ui-state-default'>");
		jq162("<table cellspacing='0' cellpadding='0' border='0' class='cg-navTable'>"
			+"<tbody>"
...
// Select page
		...
		div.append("</div>");
		div.appendTo(ul);
		jq162.each(self.options.rowsArray, function( index, value ) {
			jq162('.' +self.element.attr('id') +'.recordXP').append("<option value='"+ value +"' role='option'>"+ value +"</option>");
		});
		jq162('.' +self.element.attr('id') +'.recordXP').val(self.options.rows);
		if(self.page>1){
			jq162('.' +self.element.attr('id') +'.cg-keynav-first').removeClass("cg-state-disabled");
			jq162('.' +self.element.attr('id') +'.cg-keynav-prev').removeClass("cg-state-disabled");
		} else {
			jq162('.' +self.element.attr('id') +'.cg-keynav-first').addClass("cg-state-disabled");
			jq162('.' +self.element.attr('id') +'.cg-keynav-prev').addClass("cg-state-disabled");
		};
		if(self.page==total){
			jq162('.' +self.element.attr('id') +'.cg-keynav-next').addClass("cg-state-disabled");
			jq162('.' +self.element.attr('id') +'.cg-keynav-last').addClass("cg-state-disabled");
		};
		
		jq162('.' +self.element.attr('id') +'.cg-keynav-next').bind('click.combogrid',function(){
			if(self.page<total){
				self.page++;
				self._search(self.term);
			}
		});
		jq162('.' +self.element.attr('id') +'.cg-keynav-prev').bind('click.combogrid',function(){
			if(self.page>1){
				self.page--;
				self._search(self.term);
			}
		});
		jq162('.' +self.element.attr('id') +'.cg-keynav-last').bind('click.combogrid',function(){
			if( total>1 && self.page<total ){
				self.page=total;
				self._search(self.term);
			}
		});
		jq162('.' +self.element.attr('id') +'.cg-keynav-first').bind('click.combogrid',function(){
			if( total>1 && self.page>1){
				self.page=1;
				self._search(self.term);
			}
		});
		jq162('.' +self.element.attr('id') +'.currentPage').keypress(function(e) {
				var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;
				if(key == 13) {
					if(!isNaN(jq162(this).val()) && jq162(this).val()!=0){
						if(jq162(this).val()>total){
							self.page=total;
						}else{
							self.page=jq162(this).val();
						}
						self._search(self.term);
					}
				}
		});
		jq162('.' +self.element.attr('id') +'.recordXP').bind('change',function() {
	...
	_renderPagerPage: function (label, page, total){
		var self = this;
		if(this.options.i18n){
			return jq162.i18n.prop('page')+' <input type="text" size="1" class="' +self.element.attr('id')+ ' currentPage" value="'+ page + '"></input> ' + jq162.i18n.prop('of') + ' ' + total;
		} else {
	...
	_renderPagerView: function (label, initRecord, lastRecord, records){
		if(this.options.i18n){
			return jq162.i18n.prop(label, initRecord, lastRecord, records);
		} else {
...
jq162.extend( jq162.cg.combogrid, {
	escapeRegex: function( value ) {
		return value.replace(/[-[\]{}()*+?.,\\^jq162|#\s]/g, "\\jq162&");
	},
	filter: function(array, term) {
		var matcher = new RegExp( jq162.cg.combogrid.escapeRegex(term), "i" );
		return jq162.grep( array, function(value) {
	...
	_create: function() {
		...
		this.element
			...
			.click(function( event ) {
				if ( !jq162( event.target ).closest( ".cg-menu-item div" ).length ) {
					return;
				}
				// temporary
				event.preventDefault();
	...
	refresh: function() {
		...
		items.children("div")
			...
			.mouseenter(function( event ) {
				self.activate( event, jq162(this).parent() );
			})
			.mouseleave(function() {
				self.deactivate();
			});
		...
		if (this.hasScroll()) {
			...
			var base = this.active.offset().top,
				...
				result = this.element.children(".cg-menu-item").filter(function() {
					var close = jq162(this).offset().top - base - height + jq162(this).height();
